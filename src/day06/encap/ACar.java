...
public class ACar {
    ...
    // 생성자 : 객체가 처음 생성될 때 상태
    public ACar(String model) {
        this.model = model;
        this.mode = 'P';
//        this.speed = 0;   // 초기화가 0이니까 기본값설정 필요없음
//        this.start = false; // 초기화가 false니까 기본값설정 필요없음
    }

    // getter: 필드값을 참조하기 위한 메서드
    public int getSpeed() {
        // 필드에는 km로 저장되어 있는데 mile로 변환해서 줘야함. (추가예시. 주민번호 ****)
        return this.speed;
    }
    ...
    // 만약 setter를 없애버리면, 외부에서 바꾸려고해도 절대 바꿀수 없음
